...
function mapContent = mapStructRecursive(s, level, path, currentDepth, maxDepth)
    ...
    for i = 1:length(fields)
        ...
        mapContent{end+1} = sprintf('%sField: %s Nesting Level %d', repmat('    ', 1, level), fieldName, currentDepth);
        ...
        for j = 1:numel(s)
            ...
            if isstruct(value)
                % Include dimensions and number of fields for structs
                structDims = size(value);
                dimStr = sprintf('[%s]', strjoin(arrayfun(@num2str, structDims, 'UniformOutput', false), 'x'));
                subFields = length(fieldnames(value));
                mapContent{end+1} = sprintf('%s    Element%d: (Struct%s<%d>) %s Nesting Level %d', ...
                    repmat('    ', 1, level), j, dimStr, subFields, matchingPath, currentDepth+1);
                mapContent = [mapContent, mapStructRecursive(value, level+2, matchingPath, currentDepth+1, maxDepth)];
            elseif iscell(value)
                mapContent = [mapContent, mapCell(value, level+1, matchingPath, currentDepth+1, maxDepth)];
            else
                mapContent = [mapContent, mapElement(value, j, level+1, matchingPath, currentDepth+1)];
            end
        end
...
function mapContent = mapElement(value, elementNumber, level, datasetPath, nestingLevel)
    global datasetPaths;
    matchingPath = findMatchingPath(datasetPath, datasetPaths);
    if isnumeric(value) || islogical(value)
        mapContent = {sprintf('%s    Element%d: (%s: %s) %s Nesting Level %d', repmat('    ', 1, level), elementNumber, class(value), mat2str(size(value)), matchingPath, nestingLevel)};
    elseif ischar(value)
        mapContent = {sprintf('%s    Element%d: (char: %s) %s Nesting Level %d', repmat('    ', 1, level), elementNumber, mat2str(size(value)), matchingPath, nestingLevel)};
    elseif iscell(value)
        mapContent = {sprintf('%s    Element%d: (cell: %s) %s Nesting Level %d', repmat('    ', 1, level), elementNumber, mat2str(size(value)), matchingPath, nestingLevel)};
    else
        mapContent = {sprintf('%s    Element%d: (%s) %s Nesting Level %d', repmat('    ', 1, level), elementNumber, class(value), matchingPath, nestingLevel)};
    end
end
function mapContent = mapCell(c, level, path, currentDepth, maxDepth)
    ...
    mapContent{end+1} = sprintf('%s%s (Cell: %s) Nesting Level %d', repmat('    ', 1, level), matchingPath, mat2str(size(c)), currentDepth);
    for i = 1:numel(c)
        value = c{i};
        elementPath = sprintf('%s/cell_%d', matchingPath, i);
        if isstruct(value)
            mapContent{end+1} = sprintf('%sElement%d: %s (Struct) Nesting Level %d', repmat('    ', 1, level+1), i, elementPath, currentDepth+1);
            mapContent = [mapContent, mapStructRecursive(value, level+2, elementPath, currentDepth+1, maxDepth)];
        else
            mapContent = [mapContent, mapElement(value, i, level+1, elementPath, currentDepth+1)];
        end
    end
end
...
function createDataset(filePath, datasetPath, data, index)
    ...
    elseif isa(data, 'matlab.graphics.primitive.Image')
        h5create(filePath, [datasetPath '/type'], [1], 'Datatype', 'string');
        h5write(filePath, [datasetPath '/type'], "matlab.graphics.primitive.Image");
        h5create(filePath, [datasetPath '/CData'], size(data.CData), 'Datatype', class(data.CData));
        h5create(filePath, [datasetPath '/XData'], size(data.XData), 'Datatype', class(data.XData));
        h5create(filePath, [datasetPath '/YData'], size(data.YData), 'Datatype', class(data.YData));
...
function writeData(filePath, datasetPath, data, index)
    ...
    elseif isnumeric(data)
        h5write(filePath, datasetPath, data);
    elseif isa(data, 'matlab.graphics.primitive.Image')
        h5write(filePath, [datasetPath '/CData'], data.CData);
        h5write(filePath, [datasetPath '/XData'], data.XData);
        h5write(filePath, [datasetPath '/YData'], data.YData);
    elseif isobject(data)
...
function mapContent = generateStructMap(structData, structName)
    ...
    mapContent = {};
    mapContent{end+1} = sprintf('%s (Struct Array[1x%d]) Nesting Level 1', structName, numel(structData));
